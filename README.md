...
- Executar os testes
- Gerar o arquivo JAR em `target/itauDesafio-0.0.1-SNAPSHOT.jar`
...
```bash
java -jar target/itauDesafio-0.0.1-SNAPSHOT.jar
```

### Opção 3: Executando a classe principal
```bash
./mvnw exec:java -Dexec.mainClass="dev.iago.itauDesafio.ItauDesafioApplication"
```
...
```bash
./mvnw test -Dtest=ItauDesafioApplicationTests
```
...
1. Parar a aplicação que está usando a porta
2. Ou alterar a porta em `application.yml` adicionando:
```yaml
server:
  port: 8081
```
